...
const env = {};
const essentials = /^(PATH|SYSTEMROOT|SYSTEMDRIVE|WINDIR|TEMP|TMP|HOME|USERPROFILE|LOCALAPPDATA|APPDATA|PATHEXT|NUMBER_OF_PROCESSORS|PROCESSOR_ARCHITECTURE|PLAYWRIGHT_BROWSERS_PATH)$/i;
for (const [key, value] of Object.entries(process.env)) if (essentials.test(key)) env[key] = value;
Object.assign(env, {
  NODE_ENV: compiled ? 'production' : 'development', CI: 'true', NEXT_TELEMETRY_DISABLED: '1',
  FSC_LOCAL_PREVIEW: '1', LEAD_DELIVERY_MODE: 'local', NEXT_PUBLIC_PLAUSIBLE_SCRIPT_URL: '',
...
await startServer({ dir: root, isDev: !compiled, hostname: '127.0.0.1', port: Number(port), allowRetry: false });
